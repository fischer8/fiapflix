...
export default function Card(props) {
  ...
  return (
    <section className="bg-white rounded-lg shadow-md p-4 relative" id={card.id}>
        {card.fav ?
          <BookmarkIcon onClick={() => handleFav(card.id, false)} className="absolute right-3 top-2 hover:text-pink-100" fill='red' width={50} />
          :
          <BookMarkOutline onClick={() => handleFav(card.id, true)} className="absolute right-3 top-2 hover:text-pink-600" fill='grey' width={50} />
        }
